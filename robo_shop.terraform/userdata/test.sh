...
#####use "localhost" for Services if hosted on same server, if not change "localhost" to respective services' IP address or DNS name.#####
 ...
 location /api/catalogue/ { proxy_pass http://localhost:80/; }
 location /api/user/ { proxy_pass http://localhost:80/; }
 location /api/cart/ { proxy_pass http://localhost:80/; }
 location /api/shipping/ { proxy_pass http://localhost:80/; }
 location /api/payment/ { proxy_pass http://localhost:80/; }
